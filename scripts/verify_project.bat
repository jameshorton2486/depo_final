...
%PYTHON_BIN% -c "import backend.app; import backend.database.init_db; import backend.database.repositories.cases_repo; import desktop.launcher"
...
echo [4/6] Verifying FastAPI startup and DB status...
%PYTHON_BIN% -c "import json, subprocess, time, urllib.request; process = subprocess.Popen(['.\\\\.venv\\\\Scripts\\\\python.exe', '-m', 'uvicorn', 'backend.app:app', '--host', '127.0.0.1', '--port', '8765']); time.sleep(2); health = json.loads(urllib.request.urlopen('http://127.0.0.1:8765/api/health').read().decode('utf-8')); db_status = json.loads(urllib.request.urlopen('http://127.0.0.1:8765/api/db/status').read().decode('utf-8')); frontend = urllib.request.urlopen('http://127.0.0.1:8765/').status; process.terminate(); process.wait(timeout=10); assert health['status'] == 'ok'; assert db_status['database'] == 'connected'; assert db_status['tables_initialized'] is True; assert frontend == 200; print(health); print(db_status)"
if errorlevel 1 exit /b 1
